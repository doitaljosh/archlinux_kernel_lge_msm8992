...
void packagelist_lock_init(void *g_packagelist_data)
{
	struct global_packagelist_data *g_pkgl_dat = (struct global_packagelist_data *)g_packagelist_data;
#if !USE_SPIN// use spin
	mutex_init(&g_pkgl_dat->hashtable_lock);
#else
	spin_lock_init(&g_pkgl_dat->hashtable_lock);
#endif
}
void packagelist_lock(void *g_packagelist_data)
{
	struct global_packagelist_data *g_pkgl_dat = (struct global_packagelist_data *)g_packagelist_data;
#if !USE_SPIN// use spin
	mutex_lock(&g_pkgl_dat->hashtable_lock);
#else
	spin_unlock(&g_pkgl_dat->hashtable_lock);
#endif
}

/* This function used for get_derived_permission_recursive() */
void packagelist_unlock(void *g_packagelist_data)
{
	struct global_packagelist_data *g_pkgl_dat = (struct global_packagelist_data *)g_packagelist_data;
#if !USE_SPIN// use spin
	mutex_unlock(&g_pkgl_dat->hashtable_lock);
#else
	spin_unlock(&g_pkgl_dat->hashtable_lock);
#endif
}
...
void* get_pkgl_dat_pkglid(void *pkgl_id)
{
    int     i;
    ...
    for (i=0; i < MAX_PKGL; i++)
    {
        ...
        // find one.
		if (g_pkgls[i]->pkgl_id == (struct packagelist_data *)pkgl_id)
            return (void *)g_pkgls[i];
    }
...
static void free_pkgl_dat(struct work_struct *work)
{
	struct global_packagelist_data * g_pkg =
		container_of(to_delayed_work(work), struct global_packagelist_data, free_work);
...
static int read_package_list(struct packagelist_data *pkgl_dat) {
	...
	g_pkgl_dat = get_pkgl_dat_pkglid(pkgl_dat);
	if (g_pkgl_dat==NULL)
		return -EACCES;

	packagelist_lock(g_pkgl_dat);
	if (((struct global_packagelist_data *)g_pkgl_dat)->pkgl_id == NULL){
...
void * packagelist_create(const char *dev_name, struct super_block *sb)
{
	...
	if (!g_pkgl) {
		...
		packagelist_thread = kthread_create(packagelist_reader, (void *)pkgl_dat, "pkgld");
		...
		put_pkgl_dat(g_pkgl);
		wake_up_process(packagelist_thread);
		...
		pkg_super->sb = sb;
		pkg_super->type = ((struct sdcardfs_sb_info *)sb->s_fs_info)->options.type;
		packagelist_lock(g_pkgl);
		list_add_tail(&pkg_super->list, &pkgl_dat->pkg_supers.list);
		packagelist_unlock(g_pkgl);
	}
...
void packagelist_destroy(void *pkgl_id, type_t type)
{
	...
	if (g_pkgl != NULL)
	{
		...
		if (g_pkgl->access_no == 0)
		{
			...
			pkgl_dat = NULL;
			packagelist_unlock(g_pkgl);
			INIT_DELAYED_WORK(&g_pkgl->free_work, free_pkgl_dat);
			schedule_delayed_work(&g_pkgl->free_work, msecs_to_jiffies(5000));
